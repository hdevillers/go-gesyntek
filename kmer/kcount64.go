...
func NewKCount64(K int, c bool) *KCount64 {
	...
	if c {
		kcs.RcConvert = make([][]uint64, 2)
		kcs.RcConvert[0] = make([]uint64, 256)
		kcs.RcConvert[1] = make([]uint64, 256)
		n := 2*(K-32) - 2
		kcs.RcConvert[0]['C'] = uint64(2) << n
		kcs.RcConvert[0]['c'] = uint64(2) << n
		kcs.RcConvert[0]['G'] = uint64(1) << n
		kcs.RcConvert[0]['g'] = uint64(1) << n
		kcs.RcConvert[0]['A'] = uint64(3) << n
		kcs.RcConvert[0]['a'] = uint64(3) << n
		kcs.RcConvert[1]['C'] = uint64(2) << 30
		kcs.RcConvert[1]['c'] = uint64(2) << 30
		kcs.RcConvert[1]['G'] = uint64(1) << 30
		kcs.RcConvert[1]['g'] = uint64(1) << 30
		kcs.RcConvert[1]['A'] = uint64(3) << 30
		kcs.RcConvert[1]['a'] = uint64(3) << 30
	}
...
func (kcs *KCount64) Count(seq *[]byte) error {
	...
	if kcs.Canonical {
		for iSeq := range len(seqSpl.SeqSplit) {
			...
			for i := 0; i < kcs.SubK; i++ {
				...
				wrc.w1 = (wrc.w1 >> 2) | kcs.RcConvert[0][seqSpl.SeqSplit[iSeq][i+32]]
			}
			// Second word
			for i := kcs.SubK; i < kcs.K; i++ {
				w.w2 = (w.w2 << 2) | kcs.Convert[seqSpl.SeqSplit[iSeq][i]]
				wrc.w2 = (wrc.w2 >> 2) | kcs.RcConvert[1][seqSpl.SeqSplit[iSeq][i-kcs.SubK]]
			}
			...
			for i := kcs.K; i < len(seqSpl.SeqSplit[iSeq]); i++ {
				...
				w.w2 = (w.w2 << 2) | kcs.Convert[seqSpl.SeqSplit[iSeq][i]]
				wrc.w1 = (wrc.w1 >> 2) | kcs.RcConvert[0][seqSpl.SeqSplit[iSeq][i]]
				wrc.w2 = (wrc.w2 >> 2) | kcs.RcConvert[1][seqSpl.SeqSplit[iSeq][i-kcs.SubK]]
				lab[iLab] = MinKlab64(w, wrc)
				iLab++
			}
	...
	iKmer := 0
	for i := 0; i < nKmers; {
		tmpLab[iKmer][0] = lab[i].w1
		tmpLab[iKmer][1] = lab[i].w2
		cnt := 1
